...
#include "ui_znovacontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->znovaTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->znovaTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zNOVA.";
            else if (!mint.isSeedCorrect)
                strReason = "The zNOVA seed used to mint this zNOVA is not the same as currently hold in the wallet";
            else
