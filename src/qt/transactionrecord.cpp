...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZNOVAModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->znovaTracker->HasMintTx(hash))) {
            //zNOVA stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZNOVA;
...
bool IsZNOVAType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZNOVA:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZNOVA || type == TransactionRecord::MNReward) {
