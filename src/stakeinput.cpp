...
// The zNOVA block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zNOVA is a hash of the serial
...
{
    //Create an output returning the zNOVA that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZNOVAOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zNOVA output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zNOVA", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZNOVAOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zNOVA output", __func__);
...
{
    CzNOVATracker* znovaTracker = pwallet->znovaTracker.get();
    CMintMeta meta;
    if (!znovaTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    znovaTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a NOVA stake is the outpoint
