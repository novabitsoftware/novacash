...
public:
    ...
    bool WriteZNOVASeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZNOVASeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZNOVASeed_deprecated(uint256& seed);
    bool EraseZNOVASeed();
    bool EraseZNOVASeed_deprecated();

    bool WriteZNOVACount(const uint32_t& nCount);
    bool ReadZNOVACount(uint32_t& nCount);
