...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that znovaspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 NOVA at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zNOVA send
enum ZerocoinSpendStatus {
    ZNOVA_SPEND_OKAY = 0,                            // No error
    ZNOVA_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNOVA_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNOVA_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNOVA_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNOVA_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNOVA_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNOVA_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNOVA_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNOVA_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZNOVA_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNOVA_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNOVA_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNOVA_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNOVA_SPENT_USED_ZNOVA = 14,                      // Coin has already been spend
    ZNOVA_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZNOVA_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZNOVAOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fznovaAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzNOVATracker> znovaTracker;
    ...
    void setZWallet(CzNOVAWallet* zwallet)
    {
        zwalletMain = zwallet;
        znovaTracker = std::unique_ptr<CzNOVATracker>(new CzNOVATracker(strWalletFile));
    }

    CzNOVAWallet* getZWallet() { return zwalletMain; }
    ...
    /** zNOVA reset */
    boost::signals2::signal<void()> NotifyzNOVAReset;
