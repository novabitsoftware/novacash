...
CzNOVAWallet::CzNOVAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing znova seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNOVASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNOVASeed_deprecated()) {
                    LogPrintf("%s: Updated zNOVA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old znova seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of znova wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNOVAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNOVACount(nCountLastUsed);
    else if (!walletdb.ReadZNOVACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNOVAWallet::Lock()
{
    seedMaster = 0;
}

void CzNOVAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNOVAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNOVA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNOVAWallet::LoadMintPoolFromDB()
{
...
void CzNOVAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNOVAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNOVAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->znovaTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNOVAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNOVA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to znovaTracker which also adds to database
    pwalletMain->znovaTracker->Add(dMint, true);
...
void CzNOVAWallet::SeedToZNOVA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNOVAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNOVAWallet::GenerateDeterministicZNOVA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNOVAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNOVAWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNOVAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
