...
CzNOVATracker::CzNOVATracker(std::string strWalletFile)
{
...
CzNOVATracker::~CzNOVATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNOVATracker::Init()
{
...
bool CzNOVATracker::Archive(CMintMeta& meta)
{
...
bool CzNOVATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNOVATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNOVATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNOVATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzNOVATracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzNOVATracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzNOVATracker::GetSerialHashes()
{
...
CAmount CzNOVATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNOVATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNOVATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNOVATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNOVATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNOVATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNOVATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNOVATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNOVATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNOVATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNOVATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNOVAWallet* zNOVAWallet)
{
    bool iszNOVAWalletInitialized = (NULL != zNOVAWallet);
    CMintMeta meta;
    ...
    if (! iszNOVAWalletInitialized)
        zNOVAWallet = new CzNOVAWallet(strWalletFile);
    meta.isSeedCorrect = zNOVAWallet->CheckSeed(dMint);
    if (! iszNOVAWalletInitialized)
        delete zNOVAWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNOVATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNOVATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNOVATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNOVATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNOVATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNOVATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzNOVAWallet* zNOVAWallet = new CzNOVAWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zNOVAWallet);
        }
        delete zNOVAWallet;
        LogPrint("zero", "%s: added %d dznova from DB\n", __func__, listDeterministicDB.size());
    }
